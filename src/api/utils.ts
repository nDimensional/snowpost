...
const collection = "st.snowpo.post" as const

type SnowpostPostRecord = {
...
export async function getPostContent(
	...
	const { content, createdAt, updatedAt } = getRecordResponse.data.value as SnowpostPostRecord

	const getBlobResponse = await agent.com.atproto.sync.getBlob({ did: repo, cid: content.ref.toString() })
	if (!getBlobResponse.success) {
		throw new ErrorResponse(404, "Failed to locate blob")
	}

	return { content: getBlobResponse.data, createdAt, updatedAt }
}
...
export async function writePostContent(
	...
	options: { signal?: AbortSignal; createdAt?: string; updatedAt?: string } = {},
) {
	const agent = await getAgent(repo)
...
export async function deletePostContent(repo: string, rkey: string) {
	const agent = await getAgent(repo)

	try {
		const deleteRecordResponse = await agent.com.atproto.repo.deleteRecord({ repo, collection, rkey })
...
const isParent = (node: mdast.Node): node is mdast.Parent => Array.isArray((node as any).children)

function* findChild<T extends mdast.Nodes = mdast.Nodes>(root: mdast.Nodes, type: string): Iterable<T> {
	if (root.type === type) {
		yield root as T
